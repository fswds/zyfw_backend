...
public class SysVolunteerOrganizationRelationController extends BaseController
{
    ...
    public TableDataInfo list(SysVolunteerOrganizationRelation sysVolunteerOrganizationRelation)
    {
        ...
        if (loginUser.getUser().getRoles().get(0).getRoleKey().contains("admin")){
            sysVolunteerOrganizationRelation.setRole("admin");
        }
        startPage();
    ...
    public AjaxResult add(@Valid @RequestBody SysVolunteerOrganizationRelation sysVolunteerOrganizationRelation) throws Exception
    {
        Long organizationId = sysVolunteerOrganizationRelation.getVolunteerId();
        if (organizationId == null){
            logger.warn("请先成为志愿者才能加入志愿组织");
            throw new Exception("请先成为志愿者才能加入志愿组织");
        }
        // LoginUser loginUser = getLoginUser();
        // sysVolunteerOrganizationRelation.setUserId(loginUser.getUserId());
        List<SysVolunteerOrganizationRelation> sysVolunteerOrganizationRelations = sysVolunteerOrganizationRelationService.selectSysVolunteerOrganizationRelationList(sysVolunteerOrganizationRelation);
        if (!CollectionUtils.isEmpty(sysVolunteerOrganizationRelations)){
            throw new Exception("请勿重复加入志愿者组织");
        }
    ...
    {
        SysVolunteerOrganizationRelation relation = sysVolunteerOrganizationRelationService.selectSysVolunteerOrganizationRelationById(ids[0]);
        if ("2".equals(relation.getStatus())){
            logger.warn("当前用户加入组织为审核通过状态不允许直接删除");
            throw new Exception("当前用户加入组织为审核通过状态不允许直接删除");
        }
