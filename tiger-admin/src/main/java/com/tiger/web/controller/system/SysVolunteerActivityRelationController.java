...
public class SysVolunteerActivityRelationController extends BaseController
{
    ...
    public TableDataInfo list(SysVolunteerActivityRelation sysVolunteerActivityRelation)
    {
        ...
        if (loginUser.getUser().getRoles().get(0).getRoleKey().contains("admin")){
            sysVolunteerActivityRelation.setUserId(loginUser.getUserId());
            sysVolunteerActivityRelation.setRole("admin");
        }
        startPage();
    ...
    public AjaxResult add(@Valid @RequestBody SysVolunteerActivityRelation sysVolunteerActivityRelation) throws Exception
    {
        ...
        if (!Objects.equals(sysVolunteerList.get(0).getStatus(), "2")){
            throw new Exception("志愿者状态非审核通过，不允许加入活动");
        }
        sysVolunteerActivityRelation.setUserId(loginUser.getUserId());
        ...
        if (!CollectionUtils.isEmpty(sysActivityRelationDtos)){
            if ("2".equals(sysActivityRelationDtos.get(0).getUserApprovalStatus())){
                throw new Exception("请勿重复加入活动");
            }
            sysVolunteerActivityRelation.setStatus("1");
            LambdaQueryWrapper<SysVolunteerActivityRelation> updateChainWrapper = new LambdaQueryWrapper<>();
            updateChainWrapper.eq(SysVolunteerActivityRelation::getActivityId,sysVolunteerActivityRelation.getActivityId());
            updateChainWrapper.eq(SysVolunteerActivityRelation::getUserId,sysVolunteerActivityRelation.getUserId());
            updateChainWrapper.eq(SysVolunteerActivityRelation::getVolunteerId,sysVolunteerActivityRelation.getVolunteerId());
            return toAjax(sysVolunteerActivityRelationService.update(sysVolunteerActivityRelation,updateChainWrapper));
        }
    ...
    public AjaxResult edit( @RequestBody SysActivityRelationDto sysActivityRelationDto)
    {
